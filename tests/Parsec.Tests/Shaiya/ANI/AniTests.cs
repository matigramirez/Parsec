...
public class AniTests
{
    ...
    public void AniMultipleReadWriteTest(string fileName)
    {
        ...
        var ani = ParsecReader.ReadFromFile<Parsec.Shaiya.Ani.Ani>(filePath);
        ani.WriteJson(jsonPath);
        var aniFromJson = ParsecReader.ReadFromJsonFile<Parsec.Shaiya.Ani.Ani>(jsonPath);

        // Check bytes
        Assert.Equal(ani.GetBytes(), aniFromJson.GetBytes());

        aniFromJson.Write(newObjPath);
        var newANi = ParsecReader.ReadFromFile<Parsec.Shaiya.Ani.Ani>(newObjPath);
