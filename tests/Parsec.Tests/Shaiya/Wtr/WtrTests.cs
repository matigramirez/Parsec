...
public class WtrTests
{
    ...
    public void WtrMultipleReadWriteTest(string fileName)
    {
        ...
        var wtr = ParsecReader.ReadFromFile<Parsec.Shaiya.Wtr.Wtr>(filePath);
        wtr.WriteJson(jsonPath);
        var wtrFromJson = ParsecReader.ReadFromJsonFile<Parsec.Shaiya.Wtr.Wtr>(jsonPath);

        // Check bytes
        Assert.Equal(wtr.GetBytes(), wtrFromJson.GetBytes());

        wtrFromJson.Write(newObjPath);
        var newWtr = ParsecReader.ReadFromFile<Parsec.Shaiya.Wtr.Wtr>(newObjPath);
