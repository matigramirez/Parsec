...
public class SkillTests
{
    ...
    public void SkillTest(string fileName, Episode episode)
    {
        ...
        var skill = ParsecReader.ReadFromFile<Parsec.Shaiya.Skill.Skill>(filePath, episode);
    ...
    public void DbSkillTest()
    {
        ...
        var dbSkill = ParsecReader.ReadFromFile<DBSkillData>(filePath);
        dbSkill.Write(outputPath);
        dbSkill.WriteJson(jsonPath);
        dbSkill.WriteCsv(csvPath);

        var outputDbSkill = ParsecReader.ReadFromFile<DBSkillData>(outputPath);
        var jsonDbSkill = ParsecReader.ReadFromJsonFile<DBSkillData>(jsonPath);
    ...
    public void DbSkillTextTest()
    {
        ...
        var skillText = ParsecReader.ReadFromFile<DBSkillText>(filePath);
        skillText.Write(outputPath);
        skillText.WriteJson(jsonPath);
        skillText.WriteCsv(csvPath);

        var outputSkillText = ParsecReader.ReadFromFile<DBSkillText>(outputPath);
        var jsonSkillText = ParsecReader.ReadFromJsonFile<DBSkillText>(jsonPath);
