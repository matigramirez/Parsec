...
public class ItmTests
{
    ...
    public void ItmMultipleReadWriteTest(string fileName)
    {
        ...
        var itm = ParsecReader.ReadFromFile<Parsec.Shaiya.Itm.Itm>(filePath);
        itm.WriteJson(jsonPath);
        var itmFromJson = ParsecReader.ReadFromJsonFile<Parsec.Shaiya.Itm.Itm>(jsonPath);

        // Check bytes
        Assert.Equal(itm.GetBytes(), itmFromJson.GetBytes());

        itmFromJson.Write(newObjPath);
        var newItm = ParsecReader.ReadFromFile<Parsec.Shaiya.Itm.Itm>(newObjPath);
